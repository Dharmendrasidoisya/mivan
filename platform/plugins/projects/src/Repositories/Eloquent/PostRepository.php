...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('projectscategories', function (Builder $query) use ($id) {
                $query->whereIn('projectscategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('projectscategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('projectscategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'projectscategories', 'projectscategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('projectstags', function (Builder $query) use ($stag) {
                $query->where('projectstags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentprojectsposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('projectscategories', function (Builder $query) use ($categoryId) {
                    $query->where('projectscategories.id', $categoryId);
                });
        }
    ...
    public function getAllprojectsposts(
        int $perPage = 12,
        bool $active = true,
        array $with = ['slugable']
    ): Collection|LengthAwarePaginator {
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['projectscategories'] !== null) {
            $projectscategories = array_filter((array)$filters['projectscategories']);

            $data = $data->whereHas('projectscategories', function (Builder $query) use ($projectscategories) {
                $query->whereIn('projectscategories.id', $projectscategories);
            });
        }

        if ($filters['projectscategories_exclude'] !== null) {
            $data = $data
                ->whereHas('projectscategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('projectscategories.id', array_filter((array)$filters['projectscategories_exclude']));
                });
        }
