<!-- Why Choose Us -->
{{-- <div class="container-fluid pt-4 pb-5 pt-lg-5">
    <div class="row align-items-center py-5">
        <div class="col-lg-6 mb-5 mb-lg-0 pe-lg-5">
            <div class="appear-animation" data-appear-animation="fadeInRightShorter"
                data-appear-animation-delay="0">
                <span
                    class="badge bg-gradient-light-primary-rgba-20 text-secondary rounded-pill text-uppercase font-weight-semibold text-2-5 px-3 py-2 px-4 mb-4"><span
                        class="d-inline-flex py-1 px-2">Expertise</span></span>
            </div>
            <div class="appear-animation" data-appear-animation="fadeInUpShorter"
                data-appear-animation-delay="200">
                <h2 class="text-9 text-lg-12 font-weight-semibold line-height-1 mb-4">To Make uPVC &
                    Aluminum Windows & Doors</h2>
            </div>
            <div class="appear-animation" data-appear-animation="fadeInUpShorter"
                data-appear-animation-delay="400">
                <p>SHINER MACHINERY has been built on engineering excellence crafted through immense
                    dedication, innovation and a constant objective to serve the global market & a decade
                    young industry expertise.</p>
            </div>
            <div class="appear-animation" data-appear-animation="fadeInUpShorter"
                data-appear-animation-delay="600">
                <div class="d-flex pt-3 align-items-center">


                    
                    <a href="" class="btn btn-rounded btn-dark box-shadow-7 font-weight-medium btn-swap-1"
                        data-clone-element="1">
                        <span>Contact Us <i
                                class="fa-solid fa-arrow-right ms-2 p-relative left-10"></i></span>
                    </a>
                </div>
            </div>
        </div>
        <div class="col-lg-6 p-relative">
            <div class="accordion accordion-modern-status accordion-modern-status-arrow accordion-modern-status-arrow-dark"
                id="accordionWhyChooseUs">
                <div class="card card-default box-shadow-8 border-radius-2 bg-light">
                    <div class="card-header bg-transparent" id="collapseWhyChooseUsHeadingOne">
                        <h4 class="card-title m-0">
                            <a class="accordion-toggle bg-transparent text-3-5 text-color-dark font-weight-semi-bold collapsed"
                                data-bs-toggle="collapse" data-bs-target="#collapseWhyChooseUsOne"
                                aria-expanded="false" aria-controls="collapseWhyChooseUsOne">
                                Guarantee
                            </a>
                        </h4>
                    </div>
                    <div id="collapseWhyChooseUsOne" class="collapse"
                        aria-labelledby="collapseWhyChooseUsHeadingOne"
                        data-bs-parent="#accordionWhyChooseUs">
                        <div class="card-body pt-0">
                            <ul>
                                <li><p class="mb-0">15 Months Quality Warranty</p></li>
                                <li><p class="mb-0">Life Time Free Maintenance Service</p></li>
                                <li><p class="mb-0">Technical Support in 24 hours online</p></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="card card-default box-shadow-8 border-radius-2 bg-light">
                    <div class="card-header bg-transparent" id="collapseWhyChooseUsHeadingTwo">
                        <h4 class="card-title m-0">
                            <a class="accordion-toggle bg-transparent text-3-5 text-color-dark font-weight-semi-bold collapsed"
                                data-bs-toggle="collapse" data-bs-target="#collapseWhyChooseUsTwo"
                                aria-expanded="false" aria-controls="collapseWhyChooseUsTwo">
                                Consultation Service Capacity
                            </a>
                        </h4>
                    </div>
                    <div id="collapseWhyChooseUsTwo" class="collapse"
                        aria-labelledby="collapseWhyChooseUsHeadingTwo"
                        data-bs-parent="#accordionWhyChooseUs">
                        <div class="card-body pt-0">
                            <ul>
                                <li><p class="mb-0">One Door Solution for Window System</p></li>
                                <li><p class="mb-0">Free Consultant Service for Any Machines</p></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="card card-default box-shadow-8 border-radius-2 bg-light">
                    <div class="card-header bg-transparent" id="collapseWhyChooseUsHeadingThree">
                        <h4 class="card-title m-0">
                            <a class="accordion-toggle bg-transparent text-3-5 text-color-dark font-weight-semi-bold collapsed"
                                data-bs-toggle="collapse" data-bs-target="#collapseWhyChooseUsThree"
                                aria-expanded="false" aria-controls="collapseWhyChooseUsThree">
                                OEM Capacity
                            </a>
                        </h4>
                    </div>
                    <div id="collapseWhyChooseUsThree" class="collapse"
                        aria-labelledby="collapseWhyChooseUsHeadingThree"
                        data-bs-parent="#accordionWhyChooseUs">
                        <div class="card-body pt-0">
                            <div class="card-body pt-0">
                                <ul>
                                    <li><p class="mb-0">Supplier with 25 Years OEM Service</p></li>
                                    <li><p class="mb-0">Customized for Requirement</p></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card card-default box-shadow-8 border-radius-2 bg-light">
                    <div class="card-header bg-transparent" id="collapseWhyChooseUsHeadingFour">
                        <h4 class="card-title m-0">
                            <a class="accordion-toggle bg-transparent text-3-5 text-color-dark font-weight-semi-bold collapsed"
                                data-bs-toggle="collapse" data-bs-target="#collapseWhyChooseUsFour"
                                aria-expanded="false" aria-controls="collapseWhyChooseUsThree">
                                Trustworthy Quality
                            </a>
                        </h4>
                    </div>
                    <div id="collapseWhyChooseUsFour" class="collapse"
                        aria-labelledby="collapseWhyChooseUsHeadingFour"
                        data-bs-parent="#accordionWhyChooseUs">
                        <div class="card-body pt-0">
                            <ul>
                                <li><p class="mb-0">8S Management</p></li>
                                <li><p class="mb-0">Strict Monitoring and Testing</p></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div> --}}





<section class="new-common-sec its-bliss-sec rocket-lazyload entered lazyloaded"
    style="background-image: url('{{ !empty($icon1) ? RvMedia::getImageUrl(BaseHelper::clean($icon1)) : '' }}');"
    data-ll-status="loaded">
    <div class="container">
        <div class="its-bliss-con">
            <div class="ul-container">
                       @if ($title)
                <span class="small-title-1 wow fadeInUp" data-wow-delay=".2s"
                    style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInUp;font-size: 13px;">{!! BaseHelper::clean($title) !!}</span>
                    @endif
                       @if ($description)
                <h2 class="ul-section-title" data-wow-delay=".2s" style="color: #fff;">{!! BaseHelper::clean($description) !!}
                </h2>
                    @endif
                       @if ($description2)
                <p class="wow fadeInUp" data-wow-delay=".3s"
                    style="visibility: visible; animation-delay: 0.3s; animation-name: fadeInUp;">{!! BaseHelper::clean($description2) !!}</p>
                     @endif
                <div class="ul-banner-slide-btns">
                    <a href="{{ url('about-us') }}" class="ul-blog-btn">Read More <span class="icon"><i
                                class="flaticon-aro-left arrow"></i></span></a>
                </div>
            </div>
        </div>
    </div>
</section>
