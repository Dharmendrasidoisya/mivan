@php
    $bookedColor = $bookedColor ?? '#D20001';
    $holdColor = $holdColor ?? '#FFFE00';
    $legendGap = $legendGap ?? '15px';
    $availableTextColor = $availableTextColor ?? '#fff';
    $bookedTextColor = $bookedTextColor ?? '#fff';
@endphp
            ...
            <div class="row gy-5">
                ...
                <div class="col-lg-12">
                    <div class="ul-project-status-legend"
                        style="margin-bottom: 20px; display: flex; gap: {{ $legendGap }}; font-family: sans-serif;     overflow: auto;">
                        <div
                        ...
                        <div
                            ...
                            <div style="width: 20px; height: 20px; background-color: {{ $bookedColor }}; border-radius: 2px;border: 1px solid;">
                            </div>
                            <span style="font-weight: 600; font-size: 14px;">Booked</span>
                        </div>
                        <div
                            style="display: flex; align-items: center; gap: 5px; border: 1px solid ; padding: 5px 5px; border-radius: 4px;    overflow: auto;">
                            <div style="width: 20px; height: 20px; background-color: {{ $holdColor }}; border-radius: 2px;border: 1px solid;">
                            </div>
        ...
        tds.forEach(td => {
            ...
            if (d && d.plot_status) {
                ...
                if (status === 'available') {
                    ...
                    td.style.color = '{{ $availableTextColor }}';
                } else if (status === 'booked') {
                    td.style.backgroundColor = '{{ $bookedColor }}';
                    td.style.color = '{{ $bookedTextColor }}';
                } else if (status === 'hold') {
                    td.style.backgroundColor = '{{ $holdColor }}';
                    td.style.color = '#000';
                }
            }
        });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('#plotTableBody td').forEach(td => {
            td.addEventListener('mouseenter', function(e) {
                ...
                if (d?.plot_status) {
                    const status = d.plot_status.toLowerCase();
                    if (status === 'booked') statusColor = '{{ $bookedColor }}';
                    else if (status === 'hold') statusColor = '{{ $holdColor }}';
                    statusText = d.plot_status;
                }

                tooltip.innerHTML = `
                <div>
                    <div style="background:${statusColor};color:#000;padding:6px 10px;font-weight:bold;border:1px solid #000;">
                        Status - ${statusText}
                    </div>
